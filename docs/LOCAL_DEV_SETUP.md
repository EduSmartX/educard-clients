...
| Symptom                                                            | Fix                                                                                                                                                                                                                                          |
| ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Web app can't reach API (network error / CORS)                     | Confirm backend is running (`docker ps`) and `apps/web/.env` has `VITE_API_BASE_URL=http://localhost:8000/api`, then restart `pnpm dev` (Vite only reads `.env` at startup).                                                                 |
