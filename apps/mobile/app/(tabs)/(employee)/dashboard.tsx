  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.delay(100).springify().damping(15)} style={styles.headerContent}>
          <View style={styles.headerTop}>
            <View>
              <Text style={styles.greeting}>{getGreeting()} {getGreetingEmoji()}</Text>
              <Text style={styles.userName}>{user?.full_name ?? user?.first_name ?? 'Teacher'}</Text>
            ...
            <View style={styles.headerRight}>
              ...
              <TouchableOpacity style={styles.profileBtn} onPress={() => router.push('/(tabs)/(employee)/settings')} activeOpacity={0.8}>
                {profileImageUrl && !imgError ? (
                  <Image source={{ uri: profileImageUrl }} style={styles.profileImage} contentFit="cover" transition={200} onError={() => setImgError(true)} />
                ) : (
                  <View style={styles.profileFallback}>
                    <Text style={styles.fallbackText}>{(user?.full_name ?? user?.first_name ?? 'T').charAt(0).toUpperCase()}</Text>
                  </View>
                )}
              </TouchableOpacity>
            </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}
        ...
        <Animated.View entering={FadeInDown.delay(500).springify().damping(15)} style={styles.section}>
          <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>Quick Actions</Text><Star size={16} color="#10b981" /></View>
          <View style={styles.quickActionsGrid}>
            {quickActions.map((action, index) => (
              <Animated.View key={action.id} entering={ZoomIn.delay(550 + index * 60).springify().damping(14)} style={styles.quickActionItem}>
                <TouchableOpacity style={styles.quickActionCard} onPress={() => router.push(action.route as never)} activeOpacity={0.75}>
                  <LinearGradient colors={action.gradient} style={styles.quickActionIcon} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
...
const styles = StyleSheet.create({
  ...
  circle1: { position: 'absolute', top: -60, right: -40, width: 200, height: 200, borderRadius: 100, backgroundColor: 'rgba(255,255,255,0.12)' },
  circle2: { position: 'absolute', bottom: -80, left: -50, width: 220, height: 220, borderRadius: 110, backgroundColor: 'rgba(255,255,255,0.08)' },
  circle3: { position: 'absolute', top: 30, left: width * 0.4, width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(255,255,255,0.06)' },
  ...
  roleTag: { marginTop: 6, alignSelf: 'flex-start', backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8, fontSize: 11, fontWeight: '700', color: '#fff', textTransform: 'capitalize', overflow: 'hidden' },
  headerRight: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  notificationBtn: { width: 40, height: 40, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  profileBtn: { width: 44, height: 44, borderRadius: 14, borderWidth: 2, borderColor: 'rgba(255,255,255,0.4)', overflow: 'hidden' },
  profileImage: { width: 40, height: 40, borderRadius: 12 },
  profileFallback: { width: '100%', height: '100%', backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  ...
  quickActionCard: { alignItems: 'center', paddingVertical: 20, paddingHorizontal: 12, backgroundColor: '#fff', borderRadius: 18, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 },
  quickActionIcon: { width: 54, height: 54, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginBottom: 10 },
