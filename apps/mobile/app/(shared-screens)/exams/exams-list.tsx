...
export default function ExamsListScreen() {
  ...
  const renderExamCard = ({ item, index }: { item: Exam; index: number }) => {
    const statusColors = EXAM_STATUS_COLORS[item.status] || { bg: '#f1f5f9', text: '#64748b' };
    ...
    return (
      <Animated.View entering={FadeInDown.delay(index * 50).springify()}>
        <TouchableOpacity
          ...
          onPress={() =>
            router.push({
              pathname: '/(shared-screens)/exams/marks',
              params: {
      ...
      >
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={styles.headerSubtitle}>{sessionName || 'Session'}</Text>
          </View>
          <TouchableOpacity
            style={styles.addBtn}
            onPress={() =>
              router.push({
                pathname: '/(shared-screens)/exams/create-exam',
                params: { sessionId },
              })
            }
          >
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyText}>Create an exam to get started</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  examInfo: { flex: 1 },
  examName: { fontSize: 16, fontWeight: '600', color: '#1e293b', marginBottom: 4 },
