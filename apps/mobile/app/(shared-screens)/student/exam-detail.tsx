...
import { useExamSessionDetail, type ExamResult } from '@/features/student-portal';
        ...
        {isSchedule ? (
          ...
          <View className="mx-4 mt-4 overflow-hidden rounded-xl border border-gray-100 bg-white">
            ...
            <View className="flex-row bg-gray-50 px-3 py-2.5">
              <Text className="w-8 text-[10px] font-semibold uppercase text-gray-400">#</Text>
              <Text className="flex-1 text-[10px] font-semibold uppercase text-gray-400">
                Subject
              </Text>
              <Text className="w-20 text-[10px] font-semibold uppercase text-gray-400">Date</Text>
              <Text className="w-16 text-[10px] font-semibold uppercase text-gray-400">Time</Text>
            ...
            {detail.exams.map((exam: ExamResult, idx: number) => (
              ...
              >
                ...
                <View className="flex-1">
                  <Text className="text-sm font-medium text-gray-800">{exam.subject_name}</Text>
                ...
                <View className="w-16">
                  {exam.start_time ? (
                    <>
                      <Text className="text-xs text-gray-600">{formatTime(exam.start_time)}</Text>
                      {exam.end_time && (
                        <Text className="text-[10px] text-gray-400">
          ...
          <>
            ...
            {(() => {
              ...
              return (
                ...
                >
                  <Text className="text-xl font-bold text-white">{pct.toFixed(1)}%</Text>
            ...
            <View className="mx-4 mt-4 overflow-hidden rounded-xl border border-gray-100 bg-white">
              <View className="border-b border-gray-100 px-4 py-3">
                <Text className="text-sm font-semibold text-gray-700">📊 Subject-wise Results</Text>
              </View>
                  ...
                  <View key={exam.exam_public_id} className="border-t border-gray-50 px-4 py-3">
                    <View className="flex-row items-center justify-between">
                      ...
                      {exam.is_absent ? (
                        <View className="rounded-md bg-gray-100 px-2 py-0.5">
                          <Text className="text-[10px] font-medium text-gray-500">Absent</Text>
                        </View>
                      ) : (
                    ...
                    <View className="mt-2 h-2 overflow-hidden rounded-full bg-gray-100">
                      <View
                        ...
                        style={{ width: `${exam.is_absent ? 0 : pct}%`, backgroundColor: barColor }}
                      />
                    </View>
                  </View>
                );
