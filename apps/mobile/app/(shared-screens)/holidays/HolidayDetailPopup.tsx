      ...
      <TouchableOpacity style={styles.popupOverlay} activeOpacity={1} onPress={onClose}>
        <View style={styles.popupCard}>
          ...
          {holidays.length > 0 && (
            <LinearGradient
              colors={[
                HOLIDAY_TYPE_CONFIG[holidays[0].holiday_type]?.color || '#7c3aed',
                (HOLIDAY_TYPE_CONFIG[holidays[0].holiday_type]?.color || '#7c3aed') + 'cc',
              ]}
          ...
          {holidays.map((h, i) => {
            ...
            return (
              <View key={h.public_id} style={styles.popupItem}>
                ...
                {i < holidays.length - 1 && <View style={styles.popupDivider} />}
              </View>
            );
          })}
