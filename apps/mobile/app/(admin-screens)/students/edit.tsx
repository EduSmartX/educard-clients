...
import {
  ...
  studentFullSchema, validateField, validateAllFields,
        ...
        <ScrollView contentContainerStyle={st.form} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <Animated.View entering={FadeInDown.delay(120)}>
            <FormSection title="Basic Information" icon="👤">
              ...
              <FormInput label="Roll Number" required value={form.roll_number} onChangeText={(v) => updateField('roll_number', v)} onBlurValidate={() => blurValidate('roll_number')} error={errors.roll_number} placeholder="Enter roll number" />
              <FormInput label="Email" value={form.email} onChangeText={(v) => updateField('email', v)} onBlurValidate={() => blurValidate('email')} error={errors.email} placeholder="Enter email" keyboardType="email-address" />
              <FormInput label="Phone" value={form.phone} onChangeText={(v) => updateField('phone', v)} onBlurValidate={() => blurValidate('phone')} error={errors.phone} placeholder="Enter phone" keyboardType="phone-pad" maxLength={10} />
          ...
          <Animated.View entering={FadeInDown.delay(180)}>
            <FormSection title="Admission Information" icon="🎓">
              <FormInput label="Admission Number" value={form.admission_number} onChangeText={(v) => updateField('admission_number', v)} placeholder="Enter admission number" />
              <FormDatePicker label="Admission Date" value={form.admission_date} onChange={(v) => updateField('admission_date', v)} />
            </FormSection>
          </Animated.View>

          <Animated.View entering={FadeInDown.delay(240)}>
            <FormSection title="Guardian Information" icon="👨‍👩‍👦">
              <FormInput label="Guardian Name" value={form.guardian_name} onChangeText={(v) => updateField('guardian_name', v)} placeholder="Enter guardian name" />
              <FormInput label="Guardian Phone" value={form.guardian_phone} onChangeText={(v) => updateField('guardian_phone', v)} onBlurValidate={() => blurValidate('guardian_phone')} error={errors.guardian_phone} placeholder="Enter guardian phone" keyboardType="phone-pad" maxLength={15} />
              <FormInput label="Guardian Email" value={form.guardian_email} onChangeText={(v) => updateField('guardian_email', v)} onBlurValidate={() => blurValidate('guardian_email')} error={errors.guardian_email} placeholder="Enter guardian email" keyboardType="email-address" />
              <FormDropdown label="Relationship" options={relationOpts} value={form.guardian_relationship} onChange={(v) => updateField('guardian_relationship', v)} placeholder="Select relationship" />
            </FormSection>
          </Animated.View>

          <Animated.View entering={FadeInDown.delay(300)}>
            <FormSection title="Medical & Additional" icon="🏥">
              <FormInput label="Medical Conditions" value={form.medical_conditions} onChangeText={(v) => updateField('medical_conditions', v)} placeholder="Any medical conditions" multiline numberOfLines={3} />
              <FormInput label="Description" value={form.description} onChangeText={(v) => updateField('description', v)} placeholder="Additional notes" multiline numberOfLines={3} />
            </FormSection>
          </Animated.View>
          ...
          <Animated.View entering={FadeInDown.delay(340)}>
            <FormSection title="Emergency Contact" icon="🆘">
              ...
              <FormInput label="Emergency Contact Phone" value={form.emergency_contact_phone} onChangeText={(v) => updateField('emergency_contact_phone', v)} placeholder="Enter contact phone" keyboardType="phone-pad" maxLength={15} />
            </FormSection>
          </Animated.View>
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={updateMutation.isPending} style={st.subBtn} activeOpacity={0.8}>
            <LinearGradient colors={['#7c3aed', '#4f46e5']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={st.subGrad}>
              {updateMutation.isPending ? <ActivityIndicator color="#fff" /> : (
                <><Save size={20} color="#fff" /><Text style={st.subText}>Update Student</Text></>
              )}
            </LinearGradient>
          </TouchableOpacity>
        </ScrollView>
