...
import {
  ...
  teacherQuickSchema, teacherFullSchema, validateField, validateAllFields,
  ...
  const submitCreate = useCallback((payload: any, forceCreate: boolean) => {
    createMutation.mutate({ data: payload, forceCreate }, {
      onSuccess: (response: any) => {
        ...
        Alert.alert('✅ Success', 'Teacher created successfully!', [{ text: 'OK', onPress: () => router.back() }]);
      },
      onError: (err: any) => {
        ...
        <ScrollView ref={scrollRef} contentContainerStyle={s.form} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <Animated.View entering={FadeInDown.delay(100)}>
            <FormSection title="Basic Information" icon="👤">
              ...
              {!quickAdd && (
                <>
                  <FormInput label="Phone" value={form.phone} onChangeText={(v) => updateField('phone', v)} onBlurValidate={() => blurValidate('phone')} error={errors.phone} placeholder="Enter phone number" keyboardType="phone-pad" maxLength={10} hint="10-digit mobile number" />
          ...
          {!quickAdd && (
            <Animated.View entering={FadeInDown.delay(200)}>
              <FormSection title="Professional Details" icon="💼">
                ...
                <FormDropdown label="Supervisor" options={supervisorOptions} value={form.supervisor_email} onChange={(v) => updateField('supervisor_email', v)} error={errors.supervisor_email} placeholder="Select supervisor" searchable loading={supervisorsLoading} />
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={createMutation.isPending} style={s.submitBtn} activeOpacity={0.8}>
            <LinearGradient colors={['#7c3aed', '#4f46e5']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.submitGrad}>
              {createMutation.isPending ? <ActivityIndicator color="#fff" /> : (
                <><Save size={20} color="#fff" /><Text style={s.submitText}>Create Teacher</Text></>
              )}
            </LinearGradient>
          </TouchableOpacity>
        </ScrollView>
