...
import type { CoreClass, CoreSubject, RoleType, Department, Supervisor, LeaveType, MasterListResponse, ApiListResponse } from '@educard/shared';
...
export async function getCoreSubjects(): Promise<CoreSubject[]> {
  const res = await apiClient.get<MasterListResponse<CoreSubject>>(API_ENDPOINTS.MASTER.SUBJECTS.LIST);
  return res.data.data || res.data as any;
}

export async function getRoleTypes(): Promise<RoleType[]> {
  const res = await apiClient.get<MasterListResponse<RoleType>>(API_ENDPOINTS.MASTER.ROLE_TYPES.LIST);
  return res.data.data || res.data as any;
}

export async function getDepartments(): Promise<Department[]> {
  const res = await apiClient.get<MasterListResponse<Department>>(API_ENDPOINTS.MASTER.DEPARTMENTS.LIST);
  return res.data.data || res.data as any;
}
...
export async function getLeaveTypes(): Promise<LeaveType[]> {
  const res = await apiClient.get<MasterListResponse<LeaveType>>(API_ENDPOINTS.MASTER.LEAVE_TYPES.LIST);
  return res.data.data || res.data as any;
}
