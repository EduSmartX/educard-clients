...
export async function fetchTeacherClasses(): Promise<TeacherClass[]> {
  const res = await apiClient.get<DetailResponse<TeacherClass[]>>(`${BASE_URL}/classes/`);
...
export async function fetchHomeworkDashboard(): Promise<HomeworkDashboardStats> {
  const res = await apiClient.get<DetailResponse<HomeworkDashboardStats>>(`${BASE_URL}/dashboard/`);
  return res.data.data;
}

export async function fetchUpcomingHomework(): Promise<Homework[]> {
  const res = await apiClient.get<DetailResponse<Homework[]>>(`${BASE_URL}/upcoming/`);
  return res.data.data;
}
...
export async function fetchHomeworkList(params?: HomeworkListParams): Promise<Homework[]> {
  const res = await apiClient.get<DetailResponse<Homework[]>>(`${BASE_URL}/`, { params });
  return res.data.data;
}

export async function fetchHomeworkDetail(publicId: string): Promise<HomeworkDetail> {
  const res = await apiClient.get<DetailResponse<HomeworkDetail>>(`${BASE_URL}/${publicId}/`);
  return res.data.data;
}
...
export async function createHomework(data: HomeworkCreatePayload): Promise<HomeworkDetail> {
  const res = await apiClient.post<DetailResponse<HomeworkDetail>>(`${BASE_URL}/`, data);
  return res.data.data;
}
...
export async function deleteHomeworkAttachment(
  ...
  await apiClient.delete(`${BASE_URL}/${homeworkPublicId}/attachments/${attachmentPublicId}/`);
}
...
export async function fetchCalendarHomework(params?: CalendarParams): Promise<CalendarHomework[]> {
  const res = await apiClient.get<DetailResponse<CalendarHomework[]>>(`${BASE_URL}/calendar/`, {
    params,
  });
  return res.data.data;
}
