...
export default function TeacherTimetableScreen() {
  ...
  const { data: manageableUsers, isLoading: usersLoading } = useManageableUsers(
    'staff',
    isAdmin,
  );
  ...
  const classColorMap = useMemo(() => {
    ...
    classNames.forEach((name, idx) => {
      map.set(
        name,
        SUBJECT_COLOR_PALETTE[idx % SUBJECT_COLOR_PALETTE.length].hex,
      );
    });
    return map;
  }, [classNames]);
      ...
      <LinearGradient colors={gradient} style={headerStyles.gradient}>
        <View style={headerStyles.content}>
          <View style={headerStyles.titleRow}>
            <TouchableOpacity onPress={handleBack} style={headerStyles.backBtn}>
          ...
          {availableDays.map(day => {
            ...
            return (
              ...
              >
                ...
                {isTodayDay && (
                  <View
                    style={[styles.todayDot, isActive && styles.todayDotActive]}
                  />
                )}
              </TouchableOpacity>
            );
        ...
        {!isLoading && dayEntries.length > 0 && (
          <View style={styles.periodsContainer}>
            {dayEntries.map((entry, index) => {
              ...
              const isBreak =
                entry.slot_type === 'short_break' ||
                entry.slot_type === 'long_break';

              const classColor =
                classColorMap.get(entry.class_name ?? '') ?? '#6366f1';
              ...
              return (
                ...
                >
                  ...
                  >
                    <Text
                      style={[styles.periodBadgeText, { color: classColor }]}
                    >
