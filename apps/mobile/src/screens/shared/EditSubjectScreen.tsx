...
export default function EditSubjectScreen() {
  ...
  const route = useRoute<RouteProp<SharedStackParamList, 'SubjectEdit'>>();
  const { id } = route.params;
  const { data: subject, isLoading: detailLoading } = useSubjectDetail(
    id || '',
  );
  ...
  useEffect(() => {
    if (subject && !formLoaded) {
      ...
      setForm({
        class_id:
          s.class_info?.public_id ||
          s.class_assigned?.public_id ||
          s.class_id ||
          '',
        subject_id: String(
          s.subject_info?.id || s.subject_master?.id || s.subject_id || '',
        ),
        subject_type:
          (s.subject_type as 'core' | 'elective' | 'language') || 'core',
        teacher_id:
          s.teacher_info?.public_id ||
          s.teacher?.public_id ||
          s.teacher_id ||
          '',
        description: s.description || '',
        display_order:
          s.display_order !== undefined ? String(s.display_order) : '',
      });
      setFormLoaded(true);
    }
  }, [subject, formLoaded]);
