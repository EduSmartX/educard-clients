...
export function ChoiceItem(props: Readonly<PreferenceItemProps>) {
  ...
  return (
    <View style={styles.prefRow}>
      ...
      <TouchableOpacity
        style={[styles.dropdown, !canManage && styles.disabledOpacity]}
        onPress={() => canManage && setDropdownPref(pref)}
        activeOpacity={canManage ? 0.7 : 1}
        disabled={!canManage}
      >
        <Text style={styles.dropdownText}>
          {formatDropdownValue(currentVal) || 'Select...'}
        </Text>
    ...
    <View style={styles.prefRow}>
      ...
      >
        <Text style={styles.dropdownText} numberOfLines={1}>
          {values.length > 0
            ? values.map(formatDropdownValue).join(', ')
            : 'Select...'}
        </Text>
        <ChevronDown size={16} color={Colors.gray[500]} />
      </TouchableOpacity>
  ...
  return (
    <View style={styles.prefRow}>
      ...
      {isEditing && canManage ? (
        ...
        <TouchableOpacity
          style={[styles.textValueBox, !canManage && styles.disabledOpacity]}
          onPress={() => {
            if (!canManage) return;
        ...
        >
          <Text
            style={[styles.textValue, !currentVal && styles.textPlaceholder]}
          >
