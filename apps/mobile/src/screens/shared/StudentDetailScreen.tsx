/**
 * Student Detail Screen — shared-stack "StudentDetail"
...
export default function StudentDetailScreen() {
  const route = useRoute<RouteProp<SharedStackParamList, 'StudentDetail'>>();
  ...
  return (
    <DetailScreenShell
      ...
      avatarImageUri={getMediaUrl(student?.profile_photo_thumbnail)}
    >
      <Animated.View entering={FadeInDown.delay(100)}>
        <DetailSection title="Personal Info" icon="👤">
          ...
          <DetailRow
            label="Date of Birth"
            value={student?.user_info?.date_of_birth}
          />
          <DetailRow
            label="Blood Group"
            value={student?.user_info?.blood_group}
          />
        </DetailSection>
      </Animated.View>
      ...
      <Animated.View entering={FadeInDown.delay(200)}>
        <DetailSection title="Admission" icon="🎓">
          ...
          <DetailRow
            label="Class"
            value={student?.class_info?.class_master?.name}
          />
      ...
      <Animated.View entering={FadeInDown.delay(400)}>
        <DetailSection title="Address" icon="📍">
          <DetailRow
            label="Street"
            value={student?.user_info?.address?.street_address}
          />
          <DetailRow
            label="Address Line 2"
            value={student?.user_info?.address?.address_line_2}
          />
          <DetailRow label="City" value={student?.user_info?.address?.city} />
          <DetailRow label="State" value={student?.user_info?.address?.state} />
          <DetailRow
            label="Zip Code"
            value={student?.user_info?.address?.zip_code}
          />
          <DetailRow
            label="Country"
            value={student?.user_info?.address?.country}
          />
        </DetailSection>
      </Animated.View>
