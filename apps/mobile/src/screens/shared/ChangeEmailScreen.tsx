...
export default function ChangeEmailScreen() {
  ...
  const route = useRoute<RouteProp<SharedStackParamList, 'ChangeEmail'>>();
  ...
  const config: ChangeCredentialConfig = useMemo(
    () => ({
      ...
      sendOtp: value =>
        authApi.sendOtp({ purpose: 'EMAIL_VERIFICATION', email: value }),
      ...
      updateStore: (u, value, result) => ({
        ...
        is_email_verified:
          (result as { is_email_verified?: boolean }).is_email_verified ?? true,
      }),
