...
import type {
  ...
  MarksOverviewStudent,
  MarksOverviewSubject,
} from '../api/exams-api';
...
function PassFailBadge({ isPass }: { isPass: boolean | null }) {
  ...
  return (
    <Badge
      ...
      className={cn('text-xs', isPass && 'bg-emerald-500 hover:bg-emerald-600')}
    >
      {isPass ? 'Pass' : 'Fail'}
    </Badge>
  );
...
function MarksCell({
  ...
  if (isAbsent) {
    return <span className="text-sm font-medium text-gray-400">AB</span>;
  }
  ...
  return (
    <div className="text-center">
      <span className={cn('text-sm font-medium', isPassing ? 'text-emerald-600' : 'text-red-600')}>
        {marks}
      </span>
...
export function MarksOverviewTable({ data }: Readonly<MarksOverviewTableProps>) {
  ...
  if (subjects.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <p className="text-sm text-gray-500">No exams found for this session and class.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* Summary Stats */}
      <div className="bg-muted/30 flex items-center gap-6 rounded-lg border px-4 py-3">
...
function StudentRow({ student, subjects, index }: Readonly<StudentRowProps>) {
