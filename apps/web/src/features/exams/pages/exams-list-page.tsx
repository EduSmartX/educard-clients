...
export function ExamsListPage() {
  ...
  // ── State ────────────────────────────────
  const { filters, page, pageSize, setFilter, setFilters, setPage, setPageSize } = useFilterParams(
  ...
  const columns = useMemo(
    () =>
      createExamColumns({
        ...
        onEdit: isAdmin
          ? (e) => navigate(ROUTES.EXAMS_EDIT.replace(':id', e.public_id))
          : undefined,
        onDelete: isAdmin
          ? (e) => {
              if (showDeleted) {
        ...
        <CardHeader className="bg-muted/30 flex flex-col gap-4 border-b px-6 py-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">{pagination?.count || 0} exam(s) found</p>
            </div>
            <ActionButtonGroup>
              {/* Export Button */}
              <Button
          ...
          <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {(sessionFilter || classFilter || statusFilter) && (
              <Button
                ...
                size="sm"
                onClick={() => {
                  setFilters({ session: '', class: '', status: '' });
                }}
